...
export default function WhatsAppButton({ lang }: WhatsAppButtonProps) {
  ...
  return (
    /* The button floats outside the header, main and footer, so without a
       landmark of its own it was the one piece of content on every page
       belonging to none — an axe "region" violation. A complementary
       landmark also gives screen-reader users a way to jump straight to
       it. */
    <aside aria-label={lang === "ar" ? "تواصل سريع" : "Quick contact"}>
      <a
        href={whatsappUrl}
        target="_blank"
